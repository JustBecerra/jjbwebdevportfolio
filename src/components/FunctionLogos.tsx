    ...
    <div className="flex gap-8">
      <button className="transparent" onClick={handleTheme}>
        {changeTheme === "dark" ? (
          ...
          <FaRegSun className="hover:text-green dark:hover:text-green text-gray dark:text-white text-xl" />
        )}
      </button>
      <button className="transparent" onClick={() => setChangeLanguage((prev) => !prev)}>
        <h1 className="text-base w-[20px] dark:hover:text-green font-bold font-playfairDisplay text-gray dark:text-white hover:text-green">
          {changeLanguage ? "ES" : "EN"}
        </h1>
      </button>
      <button className="transparent font-bold hover:text-green dark:hover:text-green text-gray dark:text-white font-playfairDisplay">About</button>
